...
var favoritos = JSON.parse(localStorage.getItem('favs_iso_pneumatica') || '[]');
...
function tradSim(id, campo, fallback) {
  var k = 'simbolos.pneumatica.' + id + '.' + campo;
  var v = _t(k);
...
function inspecionar(id) {
  ...
  document.getElementById('m-descricao').innerText = tradSim(id, 'descricao', itemInspecionado.descricao || _t('simbolos.fallback_desc_pneumatica', 'Símbolo pneumático conforme norma ISO 1219.'));
...
function alternarFavorito() {
  ...
  localStorage.setItem('favs_iso_pneumatica', JSON.stringify(favoritos));
  atualizarBotaoModal();
  renderizar();
}
