document.addEventListener('DOMContentLoaded', () => {
    ...
    function performCalculations(params) {
        ...
        if ([rpm, power, fs, d1, d2, c].some(p => !p || p <= 0) || !profile || !DB?.powerTables?.[profile] || !DB?.belts?.[profile]) {
            return { success: false, error: i18n.t('polias.error_invalid_params') };
        }
        ...
        if (isNaN(angle)) return { success: false, error: i18n.t('polias.error_invalid_geometry') };
        ...
        if (correctedBeltPower <= 0) return { success: false, error: i18n.t('polias.error_invalid_params') };
        ...
        return {
            success: true,
            data: { name: params.name || i18n.t('polias.project'), rpm, power, fs, d1, d2, c, profile, rpmFinal, ratio, L, beltSpeed, angle, numBelts, shaftLoad, vibrationFreq, beltName }
        };
    }

    function runDirectCalculation() {
        if (!validateInputs(true)) return showModal(i18n.t('polias.error_highlighted'));
        const params = {
        ...
        if (result.success) {
            ...
            showModal(`${i18n.t('polias.error_calc_failed')} ${result.error}`);
            resetResults();
        }
    }

    function runReverseOptimization() {
        if (!validateInputs(false)) return showModal(i18n.t('polias.error_highlighted'));
        const params = {
    ...
    function displayOptimizationResults(solutions) {
        ...
        tbody.innerHTML = solutions.length === 0
            ? `<tr><td colspan="6">${i18n.t('polias.no_solutions')}</td></tr>`
            : solutions.map(sol => `<tr>
                <td>${sol.profile}</td><td>${sol.d1} mm</td><td>${sol.d2} mm</td>
                <td>${sol.beltName}</td><td>R$ ${sol.cost.toFixed(2)}</td>
                <td><button class="action-button" data-solution='${JSON.stringify(sol)}'>${i18n.t('polias.btn_use')}</button></td>
            </tr>`).join('');
    }
    ...
    function saveProject() {
        if (!currentResults.rpmFinal) {
            return showModal(i18n.t('polias.save_first'));
        }
        const projectName = dom.projectName.value.trim();
        if (!projectName) {
            return showModal(i18n.t('polias.name_required'));
        }
        ...
        if (existingIndex > -1) {
            showModal(i18n.t('polias.overwrite_confirm').replace('{name}', projectName), 'confirm', () => {
            ...
            localStorage.setItem('pulleyProjects', JSON.stringify(projects));
            loadProjects();
            showModal(i18n.t('polias.saved_success').replace('{name}', projectName));
        }
    }
    ...
    function loadProjects() {
        ...
        if (projects.length === 0) {
            projectList.innerHTML = `<p>${i18n.t('polias.no_projects')}</p>`;
            return;
        }
    ...
    function showModal(message, type = 'alert', callback = null) {
        ...
        dom.modalCancelBtn.textContent = type === 'confirm' ? i18n.t('btn.cancel') : i18n.t('polias.ok');
    ...
    function init() {
        if (typeof DB === 'undefined') return showModal(i18n.t('polias.error_db'));

        const formatter = (opt) => ({ value: opt.value, text: `${i18n.t('polias.fs_prefix')} ${opt.value} - ${opt.text}` });
