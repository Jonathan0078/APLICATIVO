document.addEventListener('DOMContentLoaded', () => {
    ...
    // Animation Elements
    const gear1Group = document.getElementById('gear1');
    const gear2Group = document.getElementById('gear2');
    ...
    const resultadoRpm = document.querySelector('#resultado-rpm');
     ...
     */
    const stopAnimation = () => {
        if (gear1Group) gear1Group.style.animation = 'none';
        if (gear2Group) gear2Group.style.animation = 'none';
    };
    ...
    const handleCalculateAndAnimate = () => {
        ...
        if(resultadoRelacao) resultadoRelacao.textContent = relacao.toFixed(2);
        if(resultadoRpm) resultadoRpm.textContent = rpmSaida.toFixed(2);
        if(resultadoTorque) resultadoTorque.textContent = torqueSaida.toFixed(2);
        ...
        if (rpmEntrada > 0) {
            ...
            if (gear1Group) gear1Group.style.animation = `rotate-cw ${duration1}s linear infinite`;
        }
        if (rpmSaida > 0) {
            const duration2 = 60 / rpmSaida;
            if (gear2Group) gear2Group.style.animation = `rotate-ccw ${duration2}s linear infinite`;
        }
    };
