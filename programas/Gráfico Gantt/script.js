document.addEventListener('DOMContentLoaded', () => {
    ...
    const calculateDashboardMetrics = () => {
        ...
        dashboard.innerHTML = metricCard('Progresso Geral', `${overallProgress}%`, 'var(--cor-primaria)') + metricCard('Tarefas Atrasadas', overdueCount, 'var(--cor-erro)') + metricCard('Duração (dias)', projectTotalDays, 'var(--cor-sucesso)') + metricCard('Total de Itens', tasks.length, 'var(--cor-secundaria)');
    };
    ...
    const render = () => {
        ...
        if (filteredTasks.length === 0) {
            ganttChart.innerHTML = `<div style="text-align: center; color: var(--cor-texto-suave); padding: 2rem;"><p>Nenhuma tarefa encontrada.</p></div>`;
            return;
        }
        ...
        filteredTasks.forEach(task => {
            ...
            taskEl.style.backgroundColor = task.isMilestone ? 'var(--cor-destaque)' : task.color || 'var(--cor-primaria)';
    ...
    taskForm.addEventListener('submit', (e) => {
        ...
        if (new Date(start) > new Date(end) && !isMilestone) {
            formError.textContent = 'A data de término não pode ser anterior à de início.'; return;
        }
    ...
    clearButton.addEventListener('click', () => { if(confirm('Tem a certeza que deseja limpar todas as tarefas?')) { tasks = []; saveTasks(); render(); } });
    ...
    exportXlsxButton.addEventListener('click', () => {
        const data = tasks.map(t => ({ 'ID': t.id, 'Nome': t.name, 'Início': t.start, 'Término': t.end, 'Duração (dias)': getTaskDuration(t), 'Progresso (%)': t.progress || 0, 'Marco': t.isMilestone ? 'Sim' : 'Não' }));
    ...
    // FUNÇÃO DE EXPORTAR PDF ATUALIZADA
    exportPdfButton.addEventListener('click', () => {
        // Alvo alterado para 'gantt-chart', que contém apenas as barras
        const chartElement = document.getElementById('gantt-chart');
        const originalButtonText = exportPdfButton.innerHTML;
        exportPdfButton.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> A gerar...';
        ...
        .then(canvas => {
            ...
            const pdfWidth = pdf.internal.pageSize.getWidth() - 40; // Margens de 20pt de cada lado
