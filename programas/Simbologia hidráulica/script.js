...
var favoritos = JSON.parse(localStorage.getItem('favs_iso_hidraulica') || '[]');
...
function simKey(id, campo) {
  return 'simbolos.hidraulica.' + id + '.' + campo;
}
...
function inspecionar(id) {
  ...
  document.getElementById('m-descricao').innerText = tradSim(id, 'descricao', itemInspecionado.descricao || _t('simbolos.fallback_desc_hidraulica', 'Símbolo hidráulico conforme norma ISO 1219.'));
...
function alternarFavorito() {
  ...
  localStorage.setItem('favs_iso_hidraulica', JSON.stringify(favoritos));
  atualizarBotaoModal();
  renderizar();
}
