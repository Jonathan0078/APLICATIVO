...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function popularAplicacoes() {
        resetSelect(applicationSelect, i18n.t('lms.select_app'), false);
        resetSelect(marcaSelect, i18n.t('lms.select_brand'));
        resetSelect(oleoSelect, i18n.t('lms.select_product'));
    ...
    function encontrarSubstitutos() {
        ...
        if (!marcaSelecionada || !grupoIndex) {
            resultsContainer.innerHTML = `<p class="error-message">${i18n.t('lms.err_select_all')}</p>`;
            return;
        }
    ...
    function exibirResultados(grupo, marcaOriginal) {
        ...
        let htmlResultados = `
            <h3 class="results-header" data-i18n="lms.equiv_analysis">${i18n.t('lms.equiv_analysis')}</h3>
            <div class="product-card original">
                <h4>${i18n.t('lms.your_product')}: ${produtoOriginal.NOME} (${marcaOriginal})</h4>
                <p><strong>${i18n.t('lms.application')}:</strong> ${grupo.APLICACAO}</p>
                <div class="tech-data-grid">
                    ...
                    <div class="tech-data-item"><span class="label">${i18n.t('lms.visc_index')}</span><span class="value">${produtoOriginal.IV}</span></div>
                    <div class="tech-data-item"><span class="label">${i18n.t('lms.flash_point')}</span><span class="value">${produtoOriginal.PONTO_FULGOR}°C</span></div>
                    <div class="tech-data-item"><span class="label">${i18n.t('lms.pour_point')}</span><span class="value">${produtoOriginal.PONTO_FLUIDEZ}°C</span></div>
                </div>
            </div>
            <h4 class="equivalents-title" data-i18n="lms.equivalents_title">${i18n.t('lms.equivalents_title')}</h4>
            <ul class="results-list">`;
        
        if (Object.keys(substitutos).length === 0) {
             htmlResultados += `<li>${i18n.t('lms.no_equivalent')}</li>`;
        } else {
            for (const marcaSubstituto in substitutos) {
                const produtoSubstituto = substitutos[marcaSubstituto];
                const infoCompat = matrizCompatibilidade[produtoOriginal.BASE]?.[produtoSubstituto.BASE] || { status: i18n.t('lms.unknown'), descricao: i18n.t('lms.compat_unknown') };
        ...
        htmlResultados += `</ul><div class="warning-message"><strong>${i18n.t('lms.attention')}:</strong> ${i18n.t('lms.compat_disclaimer')}</div>`;
