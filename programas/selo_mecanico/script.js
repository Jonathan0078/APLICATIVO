...
function calcularSelo() {
        ...
        if (pv <= limit) {
            pvAnalysisHTML += `
                <div style="color: var(--cor-sucesso); display: flex; align-items: center; gap: 0.5rem; font-weight: 500;">
                    <i class="fa-solid fa-circle-check"></i> <span><strong>${material}:</strong> ${i18n.t('selo.approved')} (${i18n.t('selo.pv_limit')}: ${limit})</span>
                </div>`;
            anyApproved = true;
        } else {
            pvAnalysisHTML += `
                <div style="color: var(--cor-erro); display: flex; align-items: center; gap: 0.5rem; font-weight: 500;">
                    <i class="fa-solid fa-circle-xmark"></i> <span><strong>${material}:</strong> ${i18n.t('selo.reproved')} (${i18n.t('selo.pv_limit')}: ${limit})</span>
                </div>`;
        }
    }
    pvAnalysisHTML += '</div>';
    ...
    resultsContent.innerHTML = `
        <div class="resultado-container ${overallStatusClass}">
            <h3 style="margin-bottom: 1.2rem; font-size: 1.1rem;"><i class="fa-solid fa-chart-pie"></i> ${i18n.t('selo.pv_results')}</h3>
            ...
            <div class="resultado-display" style="margin-bottom: 1.5rem; background: var(--bg); padding: 1rem; border-radius: var(--radius-sm);">
                ...
                <span class="result-sub-text" style="margin-top: 0.5rem;"><i class="fa-solid fa-compress"></i> ${i18n.t('selo.contact_pressure')} (Pc): <strong>${pc.toFixed(2)} MPa</strong></span>
                <span class="result-sub-text"><i class="fa-solid fa-gauge-high"></i> ${i18n.t('selo.velocity')} (V): <strong>${v.toFixed(2)} m/s</strong></span>
            </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 1rem; margin-bottom: 1.5rem;">
                <div style="background: var(--bg); padding: 0.8rem; border-radius: var(--radius-sm); border-left: 3px solid var(--secondary);">
                    <strong style="color: var(--text-secondary); font-size: 0.8rem; display: block; margin-bottom: 0.3rem;">${i18n.t('selo.contact_area')} (A)</strong>
                    <span style="font-weight: 700; font-size: 1.1rem; color: var(--text);">${area.toFixed(2)} mm²</span>
                </div>
                <div style="background: var(--bg); padding: 0.8rem; border-radius: var(--radius-sm); border-left: 3px solid var(--secondary);">
                    <strong style="color: var(--text-secondary); font-size: 0.8rem; display: block; margin-bottom: 0.3rem;">${i18n.t('selo.total_force')} (Fc)</strong>
                    <span style="font-weight: 700; font-size: 1.1rem; color: var(--text);">${fc.toFixed(2)} N</span>
                </div>
                <div style="background: var(--bg); padding: 0.8rem; border-radius: var(--radius-sm); border-left: 3px solid var(--secondary);">
                    <strong style="color: var(--text-secondary); font-size: 0.8rem; display: block; margin-bottom: 0.3rem;">${i18n.t('selo.hydraulic_force')} (Fh)</strong>
